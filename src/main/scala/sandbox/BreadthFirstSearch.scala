...
case class BreadthFirstSearch(board: Board, goal: Tile => Boolean) {
  ...
  case class Node(value: Tile, children: List[Node] = Nil, parent: Option[Node] = None) {
   ...
   */
  def buildGraph(tile: Tile, marked: Set[Tile] = Set(), currentLevel: Int = 0, levelLimit: Int = 3): Node = {
  ...
  def findTile(startTile: Tile): Option[Tile] = {
    val graph = buildGraph(startTile)
